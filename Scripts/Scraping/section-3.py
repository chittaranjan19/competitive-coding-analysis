...
droppedLinks = []
leaderboard = []
for i in range(60,86):	#Get all links of the leaderboard pages for section-3.
...
for oneRankList in leaderboard:	#For each page of the 25 pages
	...
	table_t = soupOfUsers.find_all('div',attrs={"class":"datatable ratingsDatatable"})
	if not(table_t == None):
		table = (table_t[0]).find_all("table")
	...
	for user in (firstRandomUser,secondRandomUser):
		...
		os.mkdir("/home/vishal/Documents/5th sem/DA/Project/Section-3/"+username) #CHANGE THIS AS PER YOUR DIRECTORY STRUCTURE
